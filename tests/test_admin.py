...
def test_urls_resolve() -> None:
    ...
    assert reverse(JOB_URLS["trigger"], args=["l", "r", "etl_job"])
    assert reverse(JOB_URLS["change"], args=["l", "r", "etl_job"])
...
class TestAuthRequired:
    ...
    def test_anonymous_job_trigger(self, client: Client) -> None:
        assert (
            client.get(
                reverse(JOB_URLS["trigger"], args=["l", "r", "etl_job"])
            ).status_code
            == 302
        )
...
class TestJobListView:
    ...
    def test_job_name_links_to_detail(
        ...
        detail_url = reverse(JOB_URLS["change"], args=["l", "r", "etl_job"])
...
class TestJobDetailView:
    ...
    def test_has_trigger_button(
        ...
        resp = staff_client.get(reverse(JOB_URLS["change"], args=["l", "r", "etl_job"]))
    ...
    def test_not_found_redirects(
        ...
        resp = staff_client.get(
            reverse(JOB_URLS["change"], args=["l", "r", "nonexistent_job"])
        )
    ...
    def test_connection_error_redirects(
        ...
        mock_get_job.side_effect = ConnectionError("refused")
    ...
    @patch("django_dagster.client.get_job")
    def test_recent_runs_calls_filter(
        self,
        mock_get_job: MagicMock,
        mock_get_runs: MagicMock,
        staff_client: Client,
        ...
        staff_client.get(reverse(JOB_URLS["change"], args=["l", "r", "etl_job"]))

        mock_get_runs.assert_called_once_with(
            job_name="etl_job", statuses=None, limit=10, cursor=None
        )
...
class TestTriggerJobView:
    def _trigger_url(
        self, location: str = "l", repository: str = "r", job_name: str = "etl_job"
    ) -> str:
        return reverse(JOB_URLS["trigger"], args=[location, repository, job_name])

    @patch("django_dagster.client.get_job_default_run_config")
    @patch("django_dagster.client.get_job")
    ...
    def test_get_default_config_error_falls_back(
        self, mock_get_job: MagicMock, staff_client: Client
    ) -> None:
    ...
    @patch("django_dagster.client.submit_job")
    @patch("django_dagster.client.get_job")
...
class TestRunListView:
    ...
    def test_renders_runs(
        ...
        mock_get_runs.assert_called_once_with(
            job_name=None, statuses=None, limit=25, cursor=None
        )
    ...
    def test_filters(
        ...
        assert resp.status_code == 200
        mock_get_runs.assert_called_once_with(
            job_name="etl_job", statuses=["FAILURE"], limit=25, cursor=None
        )
...
class TestRunDetailView:
    def _make_run_response(self, status: str = "SUCCESS") -> dict[str, Any]:
        return {
            "runOrError": {
                "__typename": "Run",
                "runId": "abc12345-def0-1234-5678-abcdef012345",
    ...
    def test_renders_detail(
        ...
        mock_get_run.return_value = self._make_run_response()["runOrError"]
        mock_get_runs.return_value = []
    ...
    def test_cancel_button_shown_for_running(
        ...
        mock_get_run.return_value = self._make_run_response(status="STARTED")[
            "runOrError"
        ]
        mock_get_runs.return_value = []
    ...
    def test_reexecute_button_shown_for_failed(
        ...
        mock_get_run.return_value = self._make_run_response(status="FAILURE")[
            "runOrError"
        ]
        mock_get_runs.return_value = []
    ...
    def test_reexecute_button_shown_for_success(
        ...
        mock_get_run.return_value = self._make_run_response(status="SUCCESS")[
            "runOrError"
        ]
        mock_get_runs.return_value = []
    ...
    @patch("django_dagster.client.get_run_events")
    @patch("django_dagster.client.get_run")
    ...
    def test_dagster_ui_job_link_shown(
        ...
        """Job Dagster UI link is built directly from run's repository/location."""
        mock_get_run.return_value = self._make_run_response()["runOrError"]
        mock_get_runs.return_value = []
...
class TestCancelRunView:
    @patch("django_dagster.client.cancel_run")
    @patch("django_dagster.client.get_run")
    ...
    @patch("django_dagster.client.get_run")
    def test_cancel_error(self, mock_get_run: MagicMock, staff_client: Client) -> None:
        mock_get_run.side_effect = Exception("Already finished")
...
class TestReexecuteRunView:
    @patch("django_dagster.client.reexecute_run")
    @patch("django_dagster.client.get_run")
    def test_reexecute_success(
        self,
        mock_get_run: MagicMock,
        mock_reexecute: MagicMock,
        staff_client: Client,
    ) -> None:
        mock_get_run.return_value = {
    ...
    def test_reexecute_error(
        self, mock_get_run: MagicMock, staff_client: Client
    ) -> None:
...
class TestPermissions:
    ...
    def test_staff_no_perms_job_detail_forbidden(self, client: Client, db: Any) -> None:
        ...
        User.objects.create_user("noperms", password="pw", is_staff=True)
        client.login(username="noperms", password="pw")
        resp = client.get(reverse(JOB_URLS["change"], args=["l", "r", "etl_job"]))
        assert resp.status_code == 403
    ...
    def test_viewer_can_see_job_detail(
        ...
        resp = viewer_client.get(
            reverse(JOB_URLS["change"], args=["l", "r", "etl_job"])
        )
        assert resp.status_code == 200
        assert b"Trigger New Run" not in resp.content

    def test_viewer_cannot_trigger_job(self, viewer_client: Client) -> None:
        resp = viewer_client.get(
            reverse(JOB_URLS["trigger"], args=["l", "r", "etl_job"])
        )
        assert resp.status_code == 403
